# -*- coding: utf-8 -*-
"""
Created on Mon May 13 09:37:46 2019
...
from decaylinear import decayMA
# from reg import reg
cpu_n=24

from new_factor.basics import *
from new_factor.stringtrans import body,repair

class FactorCompute(object):
    
    def __init__(self):
        self._global_dict={}#self._global_dict应当只是运算数据的储存,计算完毕后数据就应当被清空
        self.basic_factor = [repair(body(i)) for i in basic_factors]
    ...
    def get_value(self,name=None,defValue=None):
        try:
            ...
            try:
                value = self._global_dict[repair(body(name))]
                return value
            except:
                return defValue
    ...
    def compute(self,factor,overwrite=False,cpu_n=cpu_n):#运行因子计算前必须数据导入
        ...
        def power(df_name1, df_name2):
            ...
            if isinstance(df_name1,str) and isinstance(df_name2,str):
                ...
                # if df1.shape!=df2.shape:
                #     raise TypeError('The shape of df2 is not the same as that of df1.')
                df_m=df1.astype(float)**df2#Integers to negative integer powers are not allowed.
        ...
        def tsrank(df_name,n):
            ...
            df_m=df.rolling(n).apply(func=_rank_arr,raw=False).compute()
            """
            df_m = pd.DataFrame(data=bn.move_rank(df,window=n,axis=0),index=df.index,columns=df.columns)
            self.set_value(s,df_m)
            return s
        ...
        def var(df_name,n):
            ...
            if len(df.index)<n:
                raise TypeError('data is not sufficient')
            df_m=df.rolling(n).var()
            self.set_value(s,df_m)
            return s
        def skew(df_name,n):
            n = int(round(n))
            s = "skew(" + df_name + "," + str(n) + ")"
            if check_dup(s):
                return s
            df=self.get_value(df_name)
            if len(df.index)<n:
                raise TypeError('data is not sufficient')
            df_m=df.rolling(n).skew()
            self.set_value(s,df_m)
            return s
        def kurt(df_name,n):
            n = int(round(n))
            s = "kurt(" + df_name + "," + str(n) + ")"
            if check_dup(s):
                return s
            df=self.get_value(df_name)
            if len(df.index)<n:
                raise TypeError('data is not sufficient')
            df_m=df.rolling(n).kurt()
            self.set_value(s,df_m)
            return s
        ...
        def product(df_name,n):
            ...
            if len(df.index)<n:
                raise TypeError('data is not sufficient')
            prod=np.product
            df_m=df.rolling(window=n).apply(func=prod,raw=False).compute()
            self.set_value(s,df_m)
            return s
        ...
        def rmean(df_name):
            ...
            # df = df.astype(float)
            df2=df.mean(axis = 1)
            df1 = pd.DataFrame(np.ones(df.shape), index=df.index, columns=df.columns)
            df_m = (df1.T * df2).T
            self.set_value(s,df_m)
            return s

        def median(df_name, n):
            n = int(round(n))
            s = "median(" + df_name + "," + str(n) + ")"
            if check_dup(s):
                return s
            df=self.get_value(df_name)
            if len(df.index)<n:
                raise TypeError('data is not sufficient')
            # df = df.astype(float)
            df_m=df.rolling(window=n).median()
            self.set_value(s,df_m)
            return s

        def pctchg(df_name, n):
            n = int(round(n))
            s = "pctchg(" + df_name + "," + str(n) + ")"
            if check_dup(s):
                return s
            df=self.get_value(df_name)
            if len(df.index)<n:
        ...
        def argminimum(df_name, n):
            ...
            if len(df.index)<n:
                raise TypeError('data is not sufficient')
            df_m = pd.DataFrame(bn.move_argmin(df,window=n,axis=0),index=df.index,columns=df.columns)
            self.set_value(s,df_m)
            return s
        ...
        def argmaximum(df_name, n):
            ...
            """
            df_m = pd.DataFrame(bn.move_argmax(df,window=n,axis=0),index=df.index,columns=df.columns)
            self.set_value(s,df_m)
            return s
        ...
        def decaylinear(df_name, n):
            ...
            if check_dup(s):
                return s
            df=dd.from_pandas(self.get_value(df_name),npartitions=cpu_n).compute()
            if len(df.index)<n:
                raise TypeError('data is not sufficient')
            ...
            df_m = decayMA(df.values,n)
            df_m = pd.DataFrame(df_m,index=df.index,columns=df.columns)
            # df_m=df.rolling(window=n).apply(decaylin,raw = False).compute()
            self.set_value(s,df_m)
            return s

        def tsregressionseq(y, n, lag, retval):
            ...
            s = "tsregressionseq(" + y + "," + str(n) + "," + str(lag) + "," + str(retval) + ")"
            if check_dup(s):
                return s
            n = int(round(n))
            lag = int(round(lag))
            ...
            self.set_value(s,r)
            return s
        def tsregression(y, x, n, retval):
            # Regression model y[t]= a+b*x[t-lag]
            # The values that can be returned (resid, a, b, estimate_of_y) are represented by 0, 1, 2, 3 respectively.
            # Parameter description: y is response variable, x is independent variable, n is regression sample size
            # lag is the number of delay days, retval controls return value
            s = "tsregression(" + y + "," + x + "," + str(n) +  "," + str(retval) + ")"
            if check_dup(s):
                return s
            n = int(round(n))
            df_y=self.get_value(y)
            ...
            if df_y.shape!=df_x.shape:
                raise TypeError('y and x do not have the same shape')
            result = reg(df_y.values,df_x.values,n)
            alpha = pd.DataFrame(result[0],index = df_y.index,columns=df_y.columns)
            beta = pd.DataFrame(result[1],index = df_y.index,columns=df_y.columns)
            residual = df_y - alpha - beta*df_x
            if retval==0:
                r = alpha
            elif retval==1:
                r = beta
        ...
        def condition(field1, field2, field3):
            ...
            if isinstance(field2, str) and isinstance(field3, str):
                ...
                try:
                    ...
                    df_m = np.where(df1.values, df2, df3)
                try:
                    df_m[mask] = np.nan
                except:
                    pass
                ...
                try:
                    ...
                    df_m = np.where(df1, field2, df3)
                # print(df_m)
                # print(np.isnan(df_m).any())
                df = pd.DataFrame(mask)
                # df.to_csv('C:/Users/aning/Desktop/res.csv')
                try:
                    df_m[mask] = np.nan
                except:
                    pass
                ...
                try:
                    ...
                    df_m = np.where(df1, field2, field3)
                try:
                    df_m[mask] = np.nan
                except:
                    pass
        ...
        def wma(df_name, n):
            ...
            df_m=df.rolling(n).apply(lambda x: np.dot(x,weight))
            self.set_value(s,df_m)
            return s
        ...
        def sma(df_name, n, m):
            ...
            if len(df.index)<n:
                raise TypeError('data is not sufficient')
            a = n * 1.0 / m - 1
            r = df.ewm(com=a,adjust=False, axis=0)
            df_m=r.mean()
            self.set_value(s,df_m)
            return s
        exec(factor_trans)
        self.output(f"Factor: {factor} has been computed")
